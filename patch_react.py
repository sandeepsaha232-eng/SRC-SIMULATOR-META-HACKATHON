...
with open('Design Cyberpunk Dashboard/src/app/App.tsx', 'r') as f:
    target_content = f.read()

# Replace MACHINES and CHART_DATA consts
# with states and a useEffect fetcher inside the App component.

# First, extract everything above `const ContainerVariants`
parts = target_content.split('const containerVariants = {')
header_part = parts[0]
rest_part = 'const containerVariants = {' + parts[1]

# Redefine the interfaces to be exported and match
...
# Now modify the exported App function to include state and polling
app_func_pattern = re.compile(r'export default function App\(\) \{.*?return \(', re.DOTALL)

app_func_replacement = '''export default function App() {
  const [machines, setMachines] = useState<Machine[]>([]);
  const [chartData, setChartData] = useState<any[]>([]);
  const [sysState, setSysState] = useState({ task: 'AWAITING_AGENT', step: 0, done: false, single: '0.00', multi: '0.00', cascade: '0.00' });
  ...
  useEffect(() => {
    ...
    const fetchState = async () => {
      try {
        ...
        if (!data.fleet || data.fleet.length === 0) return;
        
        // Single/Multi/Cascade hardcoded scores matching baseline mock or live data
        setSysState({ 
          task: data.task_name || 'UNKNOWN', 
          step: data.step_count || 0, 
          done: data.done || false,
          single: '0.21', multi: '0.45', cascade: '0.74'
        });
...
full_content = new_header + rest_part
# remove MACHINES mapping from MachineGrid and inject prop if necessary
# Wait, MachineGrid is defined BEFORE App. We need to pass machines down or just read from global context?
# Actually, it's simpler to move MachineGrid and ScoreCards inside App or pass props.
# Let's replace the entire file content, it's 300 lines of standard React.

final_react_file = """import React, { useState, useEffect } from 'react';
import Masonry, { ResponsiveMasonry } from 'react-responsive-masonry';
import { motion } from 'motion/react';
import {
...
export default function App() {
  const [machines, setMachines] = useState<Machine[]>([]);
  const [chartData, setChartData] = useState<any[]>([]);
  const [sysState, setSysState] = useState({ task: 'AWAITING_AGENT', step: 0, done: false, single: '0.21', multi: '0.45', cascade: '0.74' });
...
with open('Design Cyberpunk Dashboard/src/app/App.tsx', 'w') as f:
    f.write(final_react_file)
